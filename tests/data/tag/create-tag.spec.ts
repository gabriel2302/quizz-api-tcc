...
describe('Create Tag Use Case', () => {
  ...
  it('should return a tag id on success', async () => {
    const { sut, createTagRepositorySpy } = makeSut()

    createTagRepositorySpy.result = {
      sk_tag: 'any_sk_tag'
    }

    const result = await sut.create({ tag_name: 'any_name' })

    expect(result).toEqual({ sk_tag: 'any_sk_tag' })
  })
})
